...
**File:** `docs/workflows.html` — open anytime with:
```bash
xdg-open docs/workflows.html
```

This is the single source of truth for the pipeline's components, stages, and workflows. It covers:
- System overview (Discovery → Pipeline → YouTube → Observability loop)
- `produce` stage flow with every component at each stage
- Component status table (stable / new / in-progress / planned)
- Voice system components (`voices/registry.json`, edge / prerecorded / cloned)
- `generate-image` skill integration into COMPOSE
- Dashboard workflow and video preview
- Resume/isolation map (problem → edit this file → re-run from stage)

### When to update it

**After completing any implementation**, ask the user: _"Should I update docs/workflows.html?"_ before editing. Do not update without confirmation.

Changes that warrant asking:
- New component added to any stage
- Component status changes (`in-progress` → `stable`, `planned` → `in-progress`)
- New skill integrated into the pipeline (e.g., generate-video, voice cloning)
- New pipeline stage added
- New commands added to a workflow section
- A component removed or replaced

Changes that do NOT need a diagram update:
- Bug fixes within an existing, already-documented component
- Config tweaks or parameter changes
- Refactors that don't change what components exist or how they connect

### How to update `docs/workflows.html`

The file has three things to update depending on the change:

**1. Component Status table** (most common) — find the `<tbody>` under `<h2>Component Status</h2>` and add or edit a `<tr>`. Each row pattern:
```html
<tr>
...
**2. Stage Flow diagram** — edit the Mermaid block inside `<pre class="mermaid">` under "Stage Flow + Components". Add nodes or subgraph entries for new components.

**3. Meta line** — update the `Last updated:` date in the `<div class="meta">` near the top.

### Natural-language triggers (for the assistant)
```
"open the workflow diagram"     → xdg-open docs/workflows.html
"update the workflow diagram"   → edit docs/workflows.html per the rules above (confirm first)
"mark X as stable in the diagram" → update that component's badge in the status table
"add Y component to the diagram"  → add row to status table + node to stage flow diagram
```
...
```bash
# Build outro for a channel (auto-fetches profile.png via OAuth if missing)
...
# Use a custom music file (one-time; file is also copied to configs/channels/<profile>/outro_music.mp3)
uv run pipeline outro build --profile ideal-parents-tw --music /path/to/file.mp3 --force

# Portrait format (9:16) for Shorts-oriented channel
...
Channel outro assets live under `configs/channels/<profile>/`:
- `profile.png` — auto-fetched via OAuth on first build, or drop manually
- `outro_music.mp3` — custom music; auto-discovered by `outro build` if present (no --music flag needed)
- `outro.mp4` — generated by `pipeline outro build`

TOML fields required per profile (`configs/youtube_channels.toml`):
- `display_name` — shown in the outro animation (e.g. "理想父母")
- `tagline` — shown under the name (e.g. "陪你走過每個育兒時刻")
- `outro_enabled = true` — opt-in; defaults to false

Outro attaches automatically on `pipeline publish` (before preflight). No manual step.

YouTube end screen (interactive layer on top of the static hold):
- Studio → video editor → End screen → add Subscribe + Best-for-viewer video card
- Place elements in corners (our branding is center); set start time to ~6s into end screen

```
# Natural-language triggers (for the assistant):
#   "build the outro for X channel"         → pipeline outro build --profile X
#   "rebuild the outro"                      → pipeline outro build --profile X --force
#   "change the outro music"                 → copy new file to configs/channels/X/outro_music.mp3, then rebuild
#   "check outro status"                     → pipeline outro status
#   "preview the outro"                      → open dashboard /channels page
