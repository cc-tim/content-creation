...
def render_book_page_turn_v2(
    *,
    ...
    calligraphy_page = _blank_book_canvas(spec) if page_surface == "calligraphy_texture" else None
    ...
    with tempfile.TemporaryDirectory(prefix=f"{out.stem}_frames_", dir=out.parent) as tmp:
        ...
        for idx in range(total_frames):
            ...
            if flip_count <= 2:
                ...
                source, under = _multi_page_turn_surfaces(
                    image_a,
                    image_b,
                    flip_idx,
...
def _multi_page_turn_surfaces(
    ...
    *,
    flip_count: int = 2,
    page_surface: str = "destination_preview",
    calligraphy_page: Image.Image | None = None,
) -> tuple[Image.Image, Image.Image]:
    """Resolve source/under surfaces for multi-page book turns."""
    if page_surface == "calligraphy_texture":
        if calligraphy_page is None:
            raise ValueError("calligraphy_page is required for page_surface='calligraphy_texture'")
        source = image_a if flip_idx == 0 else calligraphy_page
        under = image_b if flip_idx >= flip_count - 1 else calligraphy_page
        return source, under
    return (image_a if flip_idx == 0 else image_b, image_b)
